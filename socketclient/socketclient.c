...
typedef struct _SCK_HANDLE {
	...
	EncData                  myEncDataFunc;  //加密函数指针
	void					*encRef;
	int						encRefLen;

	DecData                   myDecDataFunc;  //解密函数指针
	void					*decRef;
	int						decRefLen;
...
{   
	...
	//传出（二级指针)   【p2】->【p1】->【V】 被调函数分配内存 通过*p2 修改p1的指向，指向被调用函数分配的内存
	//    (一级指针)           【p1】->【V】 传入一级指针，修改V的值
	*handle = sh;     //将被调函数分配内存的指针赋值给P1
...
//客户端发报文
ITCAST_FUNC_EXPORT(int)
cltSocketSend(void *handle, unsigned char *buf,  int buflen) //handle一级指针
{
	int				rv = 0;
	SCK_HANDLE		*sh = NULL;
	...
	{
		sh->buf = (unsigned char *)malloc(sizeof(char)*buflen); //handler中分配
		if (sh->buf == NULL)
		{
			rv = -6;
...
{
	//定义一个HANDLE指针接收void*指针
	SCK_HANDLE *sh = NULL;
	sh = (SCK_HANDLE *)handle;
	sh->myEncDataFunc  = encDataCallbak;  //赋值加密函数
